...
pub fn compare(source: &str, runs: u32, warmups: u32) -> Result<()> {
    ...
    if results.iter().any(|r| r.name == "pyannote-rs" && r.segments == 0) {
        println!();
...
pub fn der(max_files: u32, max_minutes: u32) -> Result<()> {
    ...
    let files = discover_files(&vox_dir, max_files, max_minutes as f64)?;
    if files.is_empty() {
        bail!("No paired wav+rttm files found in {}", vox_dir.display());
    }
    ...
    let run_dir = root.join("benchmarks").join(&run_id);
    ...
    // fluidaudio-bench binary (Swift CLI that outputs RTTM directly)
    ...
    let has_pyannote_rs = pyannote_rs_binary.exists()
        && seg_model.exists()
        && emb_model.exists();
    ...
    implementations.push((
        "speakrs CoreML FastLite",
        ImplType::Speakrs("coreml-fast-lite"),
    ));
    ...
    for (impl_name, impl_type) in &implementations {
        ...
        let (total_time, per_file_rttm) = match impl_type {
            ImplType::Speakrs(mode) => run_speakrs_batch(
                &speakrs_binary,
                mode,
                &wav_paths,
            )?,
            ImplType::FluidAudioBench => {
                // build fluidaudio-bench first
                run_cmd(
                    Command::new("swift")
            ...
            ImplType::PyannoteRs => {
                run_per_file(
                    ...
                    &files,
                    |wav| {
                        let cmd_args = [pyannote_rs_binary.to_string_lossy().to_string(),
        ...
        // score each file using speakrs::metrics
...
fn discover_files(
    ...
    let wav_dir = voxconverse_dir.join("wav");
    let rttm_dir = voxconverse_dir.join("rttm");

    let (wav_dir, rttm_dir) = if wav_dir.exists() && rttm_dir.exists() {
        (wav_dir, rttm_dir)
    } else {
        (voxconverse_dir.to_path_buf(), voxconverse_dir.to_path_buf())
    };
...
fn save_der_results(
    ...
    lines.push(format!(
        "VoxConverse DER ({} files, {total_audio_minutes:.1} min, collar={collar:.0}ms)",
        files.len()
    ));
