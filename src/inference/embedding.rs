...
impl EmbeddingModel {
    ...
    pub fn with_mode(model_path: &str, mode: ExecutionMode) -> Result<Self, ort::Error> {
        ...
        // split-backend only on CoreML (CPU fbank + GPU tail/multi-mask);
        // on CUDA the fused model runs the full pipeline on GPU
    ...
    #[cfg(feature = "coreml")]
    pub(crate) fn fbank_30s_refs(&self) -> Option<(&SharedCoreMlModel, &CachedInputShape)> {
...
/// SpeakerKit embedder: preprocessor (fbank+resnet) + embedder (masks → embeddings) + optional PLDA
#[cfg(feature = "coreml")]
pub(crate) struct SpeakerKitEmbedSession {
    preprocessor: SharedCoreMlModel,
    embedder: SharedCoreMlModel,
    plda: Option<SharedCoreMlModel>,
    cached_waveform_shape: CachedInputShape,
    cached_preproc_shape: CachedInputShape,
    cached_masks_shape: CachedInputShape,
    cached_plda_shape: CachedInputShape,
}

#[cfg(feature = "coreml")]
impl SpeakerKitEmbedSession {
    /// Try to load SpeakerKit embedder models from fixtures/models/
    pub fn load(model_path: &str) -> Option<Self> {
        let base = Path::new(model_path).parent()?;
        let preproc_path = base.join("speakerkit-preprocessor.mlmodelc");
        let embedder_path = base.join("speakerkit-embedder.mlmodelc");

        if !preproc_path.exists() || !embedder_path.exists() {
            return None;
        }

        let preprocessor = SharedCoreMlModel::load(
            &preproc_path,
            objc2_core_ml::MLComputeUnits::CPUOnly,
        ...
        let embedder = SharedCoreMlModel::load(
            &embedder_path,
        ...
        // optional PLDA projector: [1, 64, 256] → [1, 64, 128]
        let plda_path = base.join("speakerkit-plda.mlmodelc");
        ...
        // preprocessor: [1, 480000] → [1, 2998, 80]
        // embedder: [1, 2998, 80] + [1, 64, 1767] → [1, 64, 256]
        // plda: [1, 64, 256] → [1, 64, 128]
        Some(Self {
            preprocessor,
            embedder,
            plda,
            cached_waveform_shape: CachedInputShape::new("waveforms", &[1, 480_000]),
            cached_preproc_shape: CachedInputShape::new("preprocessor_output_1", &[1, 2998, 80]),
            cached_masks_shape: CachedInputShape::new("speaker_masks", &[1, 64, 1767]),
            cached_plda_shape: CachedInputShape::new("embeddings", &[1, 64, 256]),
        })
    }

    /// Embed a 30s chunk using SpeakerKit's preprocessor + embedder pipeline
    ///
    /// Returns (window_idx, speaker_idx, embedding[256], plda_embedding[128]?) tuples
    pub fn embed_chunk(
        &self,
        seg_output: &crate::inference::segmentation::SpeakerKitSegOutput,
        chunk_audio: &[f32],
    ) -> Result<Vec<(usize, usize, Vec<f32>, Option<Vec<f32>>)>, ort::Error> {
        let num_windows = seg_output.speaker_ids.len();
        let frames_per_window = 589usize;
        let num_speakers = 3usize;
        let frames_per_chunk = 1767usize; // 589 * 30 / 10
        let seconds_per_stride = 1.0; // (30 - 10) / (21 - 1)
        let frames_per_second = frames_per_window as f64 / 10.0; // 58.9

        // compute activity directly from speaker_ids (model's speaker_activity
        // output is unreliable — sparse even when speaker_ids has data)
        let mut active_pairs: Vec<(usize, usize)> = Vec::new();
        for win in 0..num_windows {
            for spk in 0..num_speakers {
                let frame_count: f32 = (0..frames_per_window)
                    .map(|f| seg_output.speaker_ids[win][[f, spk]])
                    .sum();
                if frame_count > 2.0 {
                    active_pairs.push((win, spk));
                }
            }
        }
        ...
        // step 2: build chunk indices mapping window frames → chunk timeline
        let chunk_indices: Vec<Vec<usize>> = (0..num_windows)
            .map(|win| {
                let stride_offset =
                    (win as f64 * seconds_per_stride * frames_per_second).round() as usize;
                (0..frames_per_window)
                    .map(|f| (stride_offset + f).min(frames_per_chunk - 1))
                    .collect()
            })
            .collect();

        // step 3: build scattered masks [1, 64, 1767]
        // 64 = max(21*3=63, padded to model shape)
        let total_mask_slots = 64usize;
        let mut masks = vec![0.0f32; total_mask_slots * frames_per_chunk];

        for (win, win_indices) in chunk_indices.iter().enumerate() {
            for spk in 0..num_speakers {
                let mask_idx = win * num_speakers + spk;
                if mask_idx >= total_mask_slots {
                    break;
                }
                for (f, &chunk_f) in win_indices.iter().enumerate() {
                    let speaker_val = seg_output.speaker_ids[win][[f, spk]];
                    let overlap_val = seg_output.overlapped_activity[[win, f]];
                    let filtered = speaker_val * (1.0 - overlap_val);
                    masks[mask_idx * frames_per_chunk + chunk_f] = filtered;
                }
            }
        }

        // step 4: run preprocessor on full chunk audio
        let mut waveform_buf = vec![0.0f32; 480_000];
        let copy_len = chunk_audio.len().min(480_000);
        waveform_buf[..copy_len].copy_from_slice(&chunk_audio[..copy_len]);

        let (preproc_data, _preproc_shape) = self
            .preprocessor
            .predict_cached(&[(&self.cached_waveform_shape, &waveform_buf)])
            .map_err(|e| ort::Error::new(e.to_string()))?;

        // step 5: run embedder with preprocessor output + masks
        let (emb_data, emb_shape) = self
            .embedder
            .predict_cached(&[
                (&self.cached_preproc_shape, &preproc_data),
                (&self.cached_masks_shape, &masks),
            ])
            .map_err(|e| ort::Error::new(e.to_string()))?;

        // emb_shape = [1, 64, 256]
        let emb_dim = emb_shape[2];

        // optional PLDA projection: [1, 64, 256] → [1, 64, 128]
        let plda_data = if let Some(ref plda) = self.plda {
            let (data, _) = plda
                .predict_cached(&[(&self.cached_plda_shape, &emb_data)])
                .map_err(|e| ort::Error::new(e.to_string()))?;
            Some(data)
        } else {
            None
        };
        let plda_dim = 128usize;

        let mut results = Vec::with_capacity(active_pairs.len());

        for &(win, spk) in &active_pairs {
            let mask_idx = win * num_speakers + spk;

            // raw 256-dim embedding, L2-normalized
            let start = mask_idx * emb_dim;
            let raw = &emb_data[start..start + emb_dim];
            ...
            // PLDA 128-dim embedding if available
            let plda_embedding = plda_data.as_ref().map(|data| {
                let pstart = mask_idx * plda_dim;
                data[pstart..pstart + plda_dim].to_vec()
            });

            results.push((win, spk, embedding, plda_embedding));
        }
